...
class PullsPlotter:
    ...
    def plot ( self, values : dict, options : dict ):
        ...
        items = sorted( values.items(), key=lambda kv: abs(kv[1]), reverse=True) \
                        if opts["sort_by_abs"] else list(d.items())
        ...
        ax.set_title("Pulls by Signal Region")
        ...
        plt.savefig ( self.options["pngName"], metadata=metadata )

    def show ( self ):
        from smodels_utils.plotting.mpkitty import timg
        timg ( self.options["pngName"] )
        # return fig, ax
...
if __name__ == "__main__":
    ...
    db = environ.database
    anaIds = [ "CMS-EXO-20-004" ]
    anaIds = [ "ATLAS-EXOT-2018-06" ]
