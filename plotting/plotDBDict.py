...
def isSelectedCustomFunction ( anaid, txns : List ) -> bool:
    """ a generic hook to makes it easy to select based on any requirement.
    just write the python code here. """
    mdir = Path(__file__).resolve().parent
    with open ( f"{mdir}/met_searches.py" ) as f:
        mets = eval ( f.read() )
...
class Plotter ( LoggerBase ):
    """ the meta statistics plotter, see eg
    https://smodels.github.io/validation/300/significances.png
    """
    ...
    def __init__ ( self, args : Dict ):
        ...
        :param filtervalue: filter out signal regions with
         expectedBG < filtervalue
        ...
        :param topologies: if not None, then filter for these topologies
        e.g. T2tt
        :param unscale: unscale, i.e. use the fudged bgError also for
