particleLabels = { 1000001: "X_{d}", 2000001: "X_{d}^{2}",
            1000002: "X_{u}", 2000002: "X_{u}^{2}",
            1000003: "X_{s}", 2000003: "X_{s}^{2}",
            1000004: "X_{c}", 2000004: "X_{c}^{2}",
            1000005: "X_{b}^{1}", 2000005: "X_{b}^{2}",
            1000006: "X_{t}^{1}", 2000006: "X_{t}^{2}",
            1000011: "X_{e}", 1000012: r"X_{\nu_{e}}",
            1000013: r"X_{\mu}", 1000014: r"X_{\nu_{\mu}}",
            1000015: r"X_{\tau}", 1000016: r"X_{\nu_{\tau}}",
            2000011: "X_{e}", 2000012: r"X_{\nu_{e}}",
            2000013: r"X_{\mu}", 2000014: r"X_{\nu_{\mu}}",
            2000015: r"X_{\tau}", 2000016: r"X_{\nu_{\tau}}",
            1000021: "X_{g}", 1000022: "X_{Z}^{1}",
            1000024: "X_{W}", 1000023: "X_{Z}^{2}",
            1000037: "X_{W}^{2}", 1000025: "X_{Z}^{3}",
            1000035: "X_{Z}^{3}",
            -1000001: "#bar{X}_{d}", -2000001: "#bar{X}_{d}^{2}",
            -1000002: "#bar{X}_{u}", -2000002: "#bar{X}_{u}^{2}",
            -1000003: "#bar{X}_{s}", -2000003: "#bar{X}_{s}^{2}",
            -1000004: "#bar{X}_{c}", -2000004: "#bar{X}_{c}^{2}",
            -1000005: "#bar{X}_{b}", -2000005: "#bar{X}_{b}^{2}",
            -1000006: "#bar{X}_{t}", -2000006: "#bar{X}_{t}^{2}",
            -1000011: "#bar{X}_{e}", -1000012: r"#bar{X}_{\nu}^{e}",
            -1000013: r"#bar{X}_{\mu}", -1000014: r"#bar{X}_{\nu^{\mu}",
            -1000015: "#bar{X}_{\tau}", -1000016: r"#bar{X}_{\nu}^{\tau}",
            -2000011: "#bar{X}_{e}", -2000012: r"#bar{X}_{\nu}^{e}",
            -2000013: r"#bar{X}_{\mu}", -2000014: r"#bar{X}_{\nu}^{\mu}",
            -2000015: r"#bar{X}_{\tau}", -2000016: r"#bar{X}_{\nu}^{\tau}",
            -1000021: "#bar{X}_{g}", -1000022: "#bar{X}_{Z}^{1}",
            -1000024: "#bar{X}_{W}", -1000023: "#bar{X}_{Z}^{2}",
            -1000037: "#bar{X}_{W}^{2}", -1000025: "~#bar{X}_{Z}^{3}",
            -1000035: "#bar{X}_{Z}^{3}"}
